...
export function PatentSummaryView({ data }: { data: TileData }) {
  const extraction = data.detailed as PatentExtractionResult;
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <p className="text-lg font-semibold text-gray-900">
            {extraction.claims_analysis.total_claims}
          </p>
          <p className="text-xs text-gray-500">
            {extraction.claims_analysis.independent_claims} independent
          </p>
        </div>
        <div>
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="text-center p-3 bg-purple-50 rounded-lg">
            <p className="text-2xl font-bold text-purple-600">
              {extraction.claims_analysis.independent_claims}
            </p>
            <p className="text-xs text-gray-600">Independent</p>
          </div>
          <div className="text-center p-3 bg-purple-50 rounded-lg">
            <p className="text-2xl font-bold text-purple-600">
              {extraction.claims_analysis.total_claims - extraction.claims_analysis.independent_claims}
            </p>
            ...
            {extraction.fto_relevant_data.key_limitations_for_fto.length > 0 && (
              <div className="mt-2">
                ...
                <ul className="list-disc list-inside text-xs text-gray-600 space-y-1">
                  {extraction.fto_relevant_data.key_limitations_for_fto.slice(0, 3).map((lim, idx) => (
                    <li key={idx}>{lim}</li>
                  ))}
                </ul>
              </div>
